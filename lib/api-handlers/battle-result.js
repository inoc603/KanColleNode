...
function battleResult (req, data, admiral) {
  ...
  if (data.api_get_ship) {
    ...
    record.get_ship_id = data.api_get_ship.api_ship_id
    record.get_ship_type = -1
    for (var i in gameData.shipTypes) {
      // console.log(data.api_get_ship.api_ship_type, gameData.shipTypes[i])
      if (gameData.shipTypes[i].name == data.api_get_ship.api_ship_type) {
        record.get_ship_type = gameData.shipTypes[i].id
        break
      }
    }
  }
