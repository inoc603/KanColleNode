...
function getNightBattle (req, data, admiral) {
  ...
  if (admiral.client)
    io.to(admiral.client).emit('combined_night_battle_update', send)
  else
    io.emit('combined_night_battle_update', send)
}
...
function initFriendly (data, admiral) {
  ...
  for (var i = 1; i <= 6; i++)
    if (data.api_nowhps_combined[i] != -1)
      friendly.ships[i-1].night_start_hp = data.api_nowhps_combined[i]
...
module.exports.api = ['/api_req_combined_battle/midnight_battle']
