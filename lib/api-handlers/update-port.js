...
function updateFleets (data, admiral) {
  ...
  for (var i in data) {
    ...
    // calculate anti-air
    fleet.anti_air = fleet.ships.reduce(function (fleetAA, ship) {
      return fleetAA + 
    ...
    // console.log(gameData.equipmentType)

    // calculate tracking
    var otherTra = 0
    fleet.tracking = fleet.ships.reduce(function (fleetTra, ship) {
      otherTra += ship.tracking[0]
      return fleetTra 
        + _.zip(ship.equipment).reduce(function (shipTra, slot) {
            if (slot[0] != -1) {
              var item = gameData.equipment[admiral.equipment[slot[0]].item_id]
    ...
    fleet.tracking = Math.floor(fleet.tracking+Math.sqrt(otherTra))
