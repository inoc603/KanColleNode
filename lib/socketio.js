...
module.exports = function (server) {
  ...
  .on('connection', function (socket) {
    ...
    socket.on('bind_listener', function (data) {
      ...
      socket.join('room')
      var ad = admirals.findByMixId(data.mix_id)
      ad.listener_num = data.listener_num
      io.to(data.listener_num.toString()).emit('bind_success')
    })
